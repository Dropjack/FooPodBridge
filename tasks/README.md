...
- 状态：规格核对中
- 当前阶段：任务 001 已验收；任务 002 正在冻结工程边界、服务 ABI、包内容、环境门槛与验收方法
- 当前下一步：用户明天核对任务 002 的四项简明检查点；Build Tools 路径已按实际环境修正，开始实现前仍需允许安装 7-Zip
- 当前禁止：规格确认和环境门槛解除前不创建 CMake/C++、不下载或安装依赖、不构建或部署；不访问实机与 C 盘日常 foobar2000；`D:\Dev\FooPodBridge\Ref` 永久只读
...
| 002 | [建立 x64 组件工程与服务合同](002-建立x64组件工程与服务合同/README.md) | 规格核对中 | 可安装组件、Core targets、版本化服务 ABI、包审计 |
