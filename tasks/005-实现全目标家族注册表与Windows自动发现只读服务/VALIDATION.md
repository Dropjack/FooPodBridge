...
## 2026-09-24 DUP-001 根因与 beta.12 候选

用户连接一台 Classic 时，刷新后的界面仍出现两条同名项：第一条有 119.00 GiB 容量但为 `Format pending`，第二条为 `Not mounted`。用户确认 Windows 资源管理器仍能打开该存储卷。只读 Windows PnP 查询显示这块磁盘的父链上有两层不同的 Apple 设备 ID，原发现器仅把靠近磁盘的 ID 标为已挂载，另一个父节点被误报为未挂载。beta.12 候选把同一已挂载磁盘父链上的 Apple ID 都登记为已挂载；合成测试同时要求另一台同型号设备不被隐藏。新 Release Core 的非交互发现诊断对当前设备返回 1 台已挂载、`Ready - read-only`、1862 首。此项尚待用户在 `foobar-test` 实机复验，DUP-001 不能提前标记解决。

同次刷新还暴露当前 Classic 数据库使用 624 字节 `mhit` header，导致原 Reader 拒绝并丢失曲库；Reader 的只读兼容修正与验证记录见任务 007。诊断只读取文件头和数据库结构，未修改设备内容。

用户随后在 `foobar-test` 安装 beta.12，明确回复“refresh 什么的都 ok 了，也不再显示两个了”。按用户反馈记录这台 Classic 的刷新与重复项复验通过；未附本轮截图，不扩大为所有 Windows 拓扑或所有型号已验收。DUP-001 对此物理设备已解除，任务 005 的其他设备与能力矩阵仍在进行中。
