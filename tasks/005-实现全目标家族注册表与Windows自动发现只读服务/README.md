...
2026-09-24：beta.11 实机截图再次显示两项；只读 PnP 查询确认同一已挂载磁盘父链上的两层 Apple ID 导致第二条 `Not mounted`。beta.12 已按父链修正并通过合成测试，等待用户在 `foobar-test` 复验；在此之前 DUP-001 保持未验收。

用户随后确认 beta.12 刷新正常且只剩一条设备。DUP-001 在这台 Classic 上的实机复验通过；不推断全部型号和 Windows 设备拓扑均已验证。
