...
- 状态：已批准路线
- 日期：2026-08-28
- 当前任务：`003 实现 iPod Photo 数据库往返核心`
...
- 目标：先建立可扩展的共同往返核心，再以 Photo 格式配置完整读取传统 Photo iTunesDB、保留未知记录并执行受控模型修改；不承诺一次支持全部 iPod。
- 产物：Reader/Model/Writer/Validator、格式配置边界、二进制比较器和 Photo fixture。
