...
- 当前授权：只允许冻结规格、修正已核实的 Build Tools 路径、只读环境检查和维护任务文档；用户明天确认本规格且允许安装 7-Zip 前不创建 CMake/C++、不下载或安装依赖、不构建或部署
...
5. 部署脚本只接受 `dev`，解析后的目标必须位于 `D:\Dev\FooCrate\.local\foobar-dev`；不存在任何 C 盘或自动回退路径。
...
- 输入：官方 SDK URL/SHA-256、支持 LZMA 的 7-Zip、用户批准后的 Visual Studio Build Tools CMake/CTest 绝对路径；
...
- `C:\Program Files\7-Zip\7z.exe`、x86 Program Files 路径和用户 Scoop 路径均不存在；因此尚不能执行任务 001 指定的官方 SDK LZMA 解包复核；
...
Build Tools 路径缺口已经解除，不需要为了路径名称重装整套 Visual Studio Community。当前剩余环境门槛只有 7-Zip：用户允许安装后，再执行 SDK 下载与许可复核；在此之前不进入 C++、构建、测试和打包。

## 9. 当前用户检查点

明天只需核对以下四点，不必先通读全部技术细节：

1. 首个候选版本、无假设备/无空 UI 的组件身份范围；
2. 原创 `0BSD` 服务 ABI v1、不可变接口/GUID 和 FooCrate 安全降级规则；
3. 包内容、只部署 `foobar-dev`、由用户手动导入 `foobar-test` 的边界；
4. 允许安装 7-Zip，用于复核官方 SDK 许可证并开始实现。

确认后把状态改为“可实现”，环境复核通过后再改为“实现中”。
