...
### DEC-REF-001：原版功能与交互优先

2026-09-23 用户明确：目标是 Windows x64、符合用户审美的 iPod Manager。已核实的原版成熟功能与交互默认作为实现依据，不逐项重新要求产品决定；存在明确用户差异、x64 不兼容、许可证或设备写入安全差异时说明原因。此决定不自动恢复此前明确排除的 Sync、Eject、Video 等范围，也不授权真实设备写入。

本次核对确认 Directory recovery 参数表单为本项目新增，不能称为原版功能。用户已同意按 Ref 核对后整理入口；开发参数不作为普通用户输入，无设备回归继续由 Core 测试承担。
...
`Playlists` 下方增加 `Devices`，设备下显示统一 Library 与设备播放列表，左侧各行显示名称和歌曲总数。设备曲目表顶部保持普通列表样式，不新增 Library 大标题、设备摘要条、All/Music/Audiobooks 页签或分类数量；不强制显示媒体类型列。用户通过设备上已有的有声读物 Smart Playlist 浏览对应内容，006 只读展示其已保存成员，不创建或重算规则。底层保留 Media Kind，未来导入仍由用户明确选择类型。

右上歌曲封面与播放信息保持原行为，不被设备任务覆盖；Device overview 放在右下歌词/歌曲信息区域，作为第三个条件页签，只在浏览 iPod 的设备/Library/播放列表时出现，离开该上下文隐藏。设备信息是次要信息，进入上下文不自动抢走歌词页签；离开时若正显示设备页，恢复此前的歌词/歌曲信息页。设备操作和浏览不能改变或中断歌曲播放。

状态：2026-09-21 用户明确确定顶部/左栏简化方向及右下条件设备页签，替代初始草案；这属于设计批准，不等于实现已验收。
