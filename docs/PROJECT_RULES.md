...
- 禁止使用 computer-use、sky、cua 或其他桌面/浏览器界面控制工具；不得改用脚本、辅助功能 API、模拟输入或自动截图绕过限制。
- 所有需要打开软件、验证组件加载、操作界面或检查实际使用流程的测试均交给用户；Codex 不代为启动或操作应用。提供中文步骤，写清指定实例、入口、动作、预期结果和需要反馈的信息，一次指导一个检查点，收到结果后继续下一项相关检查。
- 代码审查、命令行构建、非交互单元/fixture 测试和包审计仍可在已批准范围内执行；它们不能代替人工界面和使用验证。未执行的人工检查必须保留待验证状态。
- 上述用户于 2026-09-20 明确规定的限制覆盖旧任务中自动操作应用、UI 检查和开发实例冒烟测试的授权。
- 纯 Core 单元测试、数据库 fixture、格式往返和故障注入从 FooPodBridge 构建目录运行，不需要启动 foobar2000。
- 开发部署与非交互诊断只使用 `D:\dev\foo\FooCrate\.local\foobar-dev`；该实例的组件加载、两组件界面通信和使用检查由用户操作。
