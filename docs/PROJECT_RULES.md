...
- 正式允许写入的设备只包括项目维护者拥有并逐台验证的 iPod Photo 和 iPod Classic。
- 未识别、能力不明或未经实机验证的型号可以显示为 Unsupported，但所有写入命令必须在服务层拒绝。
...
- 弹出、关闭、取消和设备移除必须有明确状态机。任何 UI 都不能绕过事务服务强制弹出或并发写入。
- 第一次对每台实机执行写入前，当前任务必须验证可恢复备份、设备身份和测试曲目；普通自动化或 UI 测试不构成实机写入授权。
...
- 项目公开许可证在用户批准前保持未发布状态；该决定阻止公开发布，但不阻止只包含原创文档的本地规格工作。
...
- 当前任务 000 完成全部阻断性产品决定并批准路线前，不创建 CMake、C++、组件 DLL 或 UI 脚手架。
