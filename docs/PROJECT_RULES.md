...
本项目实现一个 foobar2000 x64 组件，用现代 C++ 重建 Windows 可挂载存储卷模式非 iPod touch 的读取、手动音乐管理和安全数据库写入能力。`foo_dop / iPod manager` 只作为经过历史实机验证的知识与行为参考；正式工程不继承其 x86 二进制依赖、iPhone/iPod touch 路径或旧 UI 架构。
...
- 强制目标范围是所有能被 Windows 暴露为可访问存储卷的非 iPod touch：早期全尺寸 iPod、Mini、Photo/Color/Video/Classic、各代 Nano 和 Shuffle。每个候选卷默认进入自动发现、正向识别与能力分类；缺少维护者实机、fixture 或尚未实现某个格式不能把该家族静默移出目标，只能降低证据等级和可用能力。
- 设备兼容按数据库家族和证据等级管理，不再把“项目维护者是否刚好把每个型号带在身边”作为实现门槛。公开支持表必须区分参考支持、fixture 往返、实机只读和实机写入验证。
- 实机与私有 fixture 只验证其数据库/签名/profile 变体和设备能力，不能成为针对某台 Nano、Classic 或其他具体机器的产品配置。生产代码必须通过家族注册表、格式 profile 和能力矩阵路由，不能用设备名称、当前样本字节或单台稳定 ID 硬编码通用行为。
...
- 产品只管理已由 Windows 暴露为可访问存储卷的非 iPod touch。若用户、设备或外部软件关闭“用作磁盘”，或设备默认不暴露存储卷，FooPodBridge 只报告前置条件未满足；不负责更改该设置、启动或控制 iTunes，也不发送私有设备配置命令。
...
- 任务 000 已于 2026-09-10 按全非 touch 目标范围重开并批准修订路线；任务 001 的既有审计仍约束已实现来源。新增 Shuffle、Nano 5+ 或其他格式如需采用尚未批准的第三方表达或算法，必须先增量重开任务 001，不能把路线批准当作许可证批准。
